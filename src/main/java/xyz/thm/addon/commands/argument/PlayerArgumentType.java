package xyz.thm.addon.commands.argument;
...
public class PlayerArgumentType implements ArgumentType<PlayerEntity> {
    ...
    static {
        if (mc.world != null) {
            EXAMPLES = mc.world.getPlayers()
                .stream()
                .map(player -> player.getNameForScoreboard())
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(mc.world.getPlayers().stream().map(player -> player.getNameForScoreboard()), builder);
    }
